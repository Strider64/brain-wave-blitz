...
try {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['size'] > 0) {
        ...
        $current_image_query = "SELECT image_path FROM cms WHERE id = :id";
        ...
        $sql = "UPDATE cms SET category = :category, heading = :heading, content = :content, image_path = :image_path, thumb_path = :thumb_path, date_updated = :date_updated WHERE id = :id";
        ...
        // Prepare the SQL query with placeholders
        $sql = "UPDATE cms SET category = :category, heading = :heading, content = :content, date_updated = :date_updated WHERE id = :id";
        $stmt = $pdo->prepare($sql);
    }
