...
try {
    ...
    // Example:
    // ALTER TABLE brainwaveblitz ADD FULLTEXT(question);
    if($searchTerm !== null) {
        $sql = "SELECT * FROM brainwaveblitz WHERE MATCH(question) AGAINST(:searchTerm IN NATURAL LANGUAGE MODE) LIMIT 1";
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':searchTerm', $searchTerm);

        // If a heading was provided, search for exact matches on the 'heading' field.
    } else if($id !== null) {
        $sql = "SELECT * FROM brainwaveblitz WHERE id = :id LIMIT 1";
        $stmt = $pdo->prepare($sql);
